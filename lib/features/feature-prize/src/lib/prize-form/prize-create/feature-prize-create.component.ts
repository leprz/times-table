...
export class FeaturePrizeCreateComponent {
  ...
  constructor() {
    this.createResult$.pipe(takeUntilDestroyed()).subscribe();
  }

  private readonly createResult$: Observable<CreateOnePrizeResult> =
    this.createSubject.asObservable().pipe(
      filterNill(),
      switchMap((params) =>
        this.prizeDataService.createOne(params).pipe(
          tap(() =>
