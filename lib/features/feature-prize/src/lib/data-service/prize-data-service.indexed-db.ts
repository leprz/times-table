...
export class PrizeDataServiceIndexedDb implements PrizeDataServicePort {
  ...
  createOne(
    ...
    return fromPromise(this.db.prizeItems.add(bodyParams)).pipe(
      map(() => undefined),
    );
  }
  ...
  deleteOne(
    ...
    return fromPromise(
      this.db.prizeItems.where({ id: pathParams.id }).delete(),
    ).pipe(
      map((deleteCount) => {
        if (deleteCount === 0) {
  ...
  readMany(): Observable<ReadManyPrizesResult> {
    return from(
      this.db.prizeItems.orderBy('primaryId').reverse().toArray(),
  ...
  readOneById(
    ...
    return from(this.db.prizeItems.where({ id: pathParams.id }).first()).pipe(
      map((result) => {
        if (result) {
          return result;
        }
  ...
  findNextPrize(
    ...
    return from(
      this.db.prizeItems
        .where('requiredPoints')
        .above(bodyParams.collectedPoints)
        .limit(1)
        .first(),
    ).pipe(
      map((result) => {
